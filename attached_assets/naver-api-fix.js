// routes/api/naverShoppingInsight.js
...
/**
 * 네이버 데이터랩 쇼핑인사이트 카테고리별 인기 검색어 API 호출
 ...
 * HTTP 메서드: POST
 * 
 * @param {string} categoryId - 카테고리 ID (예: '50000000')
 * @param {string} startDate - 시작일(YYYY-MM-DD)
 * @param {string} endDate - 종료일(YYYY-MM-DD)
 * @returns {Promise<Object>} API 응답 데이터
 * 
 ...
 */
async function fetchShoppingCategoryKeywords(categoryId, startDate, endDate) {
  try {
    // 날짜 유효성 검사
    ...
    try {
      ...
      logError('대체 API 요청도 실패', retryError);
    }
    ...
    // 추가 백업 데이터 (확장된 카테고리용)
    if (!backupKeywords[backupCategory]) {
      if (backupCategory === 'food') {
        backupKeywords.food = ['김치', '과일', '쌀', '라면', '고기', '채소', '김', '간식', '빵', '과자'];
      } else if (backupCategory === 'sports') {
        backupKeywords.sports = ['운동화', '골프', '자전거', '테니스', '수영복', '등산', '헬스', '요가', '배드민턴', '캠핑'];
      } else if (backupCategory === 'health') {
        backupKeywords.health = ['비타민', '마스크', '영양제', '건강식품', '프로틴', '체중계', '혈압계', '두통약', '밴드', '안마기'];
      } else if (backupCategory === 'culture') {
        backupKeywords.culture = ['책', '영화', '음악', '게임', '콘서트', '전시', '뮤지컬', '티켓', '여행', '앨범'];
      }
    }
 ...
 */
// 응답 데이터 처리 유틸리티 함수 가져오기
const { getShoppingKeywords } = require('../../utils/shoppingInsightUtils');
...
router.get('/top-keywords', async (req, res) => {
  try {
        ...
        batch.map(async (category) => {
          try {
            ...
            return {
              ...
              keywords: keywords || [],
              isBackupData: isBackupData || false,
              error: error
            };
          } catch (error) {
            logError(`카테고리 ${category.name} 데이터 요청 오류`, error);
            return {
              categoryId: category.id,
              categoryName: category.name,
              keywords: [],
    ...
    logError('인기 검색어 조회 API 오류', error);
    res.status(500).json({
      success: false,
...
router.get('/category/:categoryId', async (req, res) => {
  try {
    ...
    logError(`카테고리(${req.params.categoryId}) 인기 검색어 조회 오류`, error);
    res.status(500).json({
      success: false,
