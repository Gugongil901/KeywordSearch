...
export function getFallbackKeywords(category: string): string[] {
  ...
  // 건강보조식품/영양제 관련 인기 키워드 모음 (2025.03 기준 전체 영양제 카테고리)
  const healthSupplementKeywords = [
    '비타민', '종합비타민', '홍삼', '홍삼정', '루테인', '오메가3', '유산균', 
    '프로폴리스', '콜라겐', '글루타치온', '마그네슘', '밀크씨슬',
    '철분', '칼슘', '비타민D', '비타민B', '아연', '코엔자임Q10',
    '크릴오일', '밀크씨슬', '쏘팔메토', '엽산', '비오틴', '지아잔틴',
    '비타민C', '비타민E', '기억력영양제', '관절영양제', '눈영양제', '간영양제',
    '멀티비타민', '프리바이오틱스', '셀레늄', '베타카로틴', '히알루론산', '브로콜리 새싹',
    '프로바이오틱스', '쏘팔메토', '속쓰림영양제', '면역영양제', '여성영양제', '남성영양제',
    '스피루리나', '엠에스엠', '칼슘마그네슘', '감마리놀렌산', '옥타코사놀', '로열젤리'
  ];

  // 경쟁사(12개) 제품 키워드 - 닥터린, 내츄럴플러스, 에스더몰, 안국건강, 고려은단, 뉴트리원, 
  // 종근당건강, GNM 자연의품격, 뉴트리데이, 주영엔에스, 한미양행, 유한양행
  const competitorKeywords = [
    // 경쟁사별 베스트셀러 제품명 추가
    '닥터린 비타민', '닥터린 루테인', '닥터린 홍삼', '닥터린 유산균', '닥터린 종합비타민',
    '내츄럴플러스 루테인', '내츄럴플러스 비타민', '내츄럴플러스 오메가3', '내츄럴플러스 콜라겐', '내츄럴플러스 마그네슘',
    '에스더몰 홍삼', '에스더몰 루테인', '에스더몰 콜라겐', '에스더몰 유산균', '에스더몰 비오틴',
    '안국건강 루테인', '안국건강 오메가3', '안국건강 밀크씨슬', '안국건강 칼슘', '안국건강 비타민D',
    '고려은단 비타민', '고려은단 홍삼', '고려은단 마그네슘', '고려은단 아연', '고려은단 쏘팔메토',
    '뉴트리원 비오틴', '뉴트리원 루테인', '뉴트리원 비타민', '뉴트리원 밀크씨슬', '뉴트리원 홍삼',
    '종근당건강 밀크씨슬', '종근당건강 루테인', '종근당건강 비타민', '종근당건강 오메가3', '종근당건강 프로폴리스',
    'GNM 비타민D', 'GNM 루테인', 'GNM 홍삼', 'GNM 유산균', 'GNM 비타민C',
    '뉴트리데이 철분', '뉴트리데이 비타민', '뉴트리데이 마그네슘', '뉴트리데이 칼슘', '뉴트리데이 홍삼',
    '주영엔에스 유산균', '주영엔에스 콜라겐', '주영엔에스 밀크씨슬', '주영엔에스 비타민', '주영엔에스 홍삼',
    '한미양행 프로폴리스', '한미양행 비타민C', '한미양행 루테인', '한미양행 유산균', '한미양행 홍삼',
    '유한양행 비타민', '유한양행 오메가3', '유한양행 루테인', '유한양행 밀크씨슬', '유한양행 유산균'
  ];
  ...
  switch (category) {
    case 'health':
      // 건강기능식품 카테고리가 선택된 경우 건강보조식품과 경쟁사 키워드 배열 병합
      return [
        ...healthSupplementKeywords.slice(0, 10),
    ...
    default: // 'all'
      // 건강기능식품 우선 + 일반 인기 키워드 혼합
      return [
        // 건강기능식품 인기 키워드 (프로젝트 주요 카테고리)
        '비타민', '종합비타민', '홍삼', '루테인', '오메가3',
        // 타 카테고리 인기 키워드
        '운동화', '선크림', '노트북', '침대', '기저귀'
      ];
  }
